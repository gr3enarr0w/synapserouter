...
verify:
  ...
  - name: "iCal unfold BEFORE field parsing"
    command: "grep -rn 'ReplaceAll.*\\\\n ' --include='*.go' -r | head -1; UNFOLD_LINE=$(grep -rn 'ReplaceAll.*\\\\n ' --include='*.go' | head -1 | cut -d: -f2); PARSE_LINE=$(grep -rn 'SUMMARY\\|DESCRIPTION.*Match\\|DTSTART' --include='*.go' | head -1 | cut -d: -f2); if [ -n \"$UNFOLD_LINE\" ] && [ -n \"$PARSE_LINE\" ] && [ \"$UNFOLD_LINE\" -lt \"$PARSE_LINE\" ]; then echo 'OK: unfold before parse'; else echo 'FAIL: must unfold continuation lines (replace \\n<space> with empty) BEFORE regex parsing fields. RFC 5545 line folding splits long lines with CRLF+space.'; fi"
    expect_not: "FAIL"
  - name: "iCal DTSTART handles timezone format"
    command: "grep -rn 'DTSTART' --include='*.go' | grep -q 'TZID\\|T[0-9]' && echo 'OK' || echo 'FAIL: DTSTART regex only matches bare dates. Must also handle DTSTART;TZID=America/New_York:20260322T080000 and DTSTART;VALUE=DATE:20260322.'"
    expect_not: "FAIL"
